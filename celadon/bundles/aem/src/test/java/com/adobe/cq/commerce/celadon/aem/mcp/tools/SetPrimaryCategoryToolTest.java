...
public class SetPrimaryCategoryToolTest {

    private final SetPrimaryCategoryTool tool = new SetPrimaryCategoryTool();

    @Test
    public void exposesStableToolName() {
        assertEquals("set_primary_category", tool.name());
    }
    ...
    public void inputSchemaIsValidJsonWithExpectedShape() {
        ...
        assertTrue("has targetCategory", props.has("targetCategory"));

        JsonArray required = schema.getAsJsonArray("required");
        assertTrue("sku is required", required.contains(JsonParser.parseString("\"sku\"")));
        assertTrue("targetCategory is required",
                required.contains(JsonParser.parseString("\"targetCategory\"")));
        assertEquals("only sku + targetCategory are required", 2, required.size());
    }
}
