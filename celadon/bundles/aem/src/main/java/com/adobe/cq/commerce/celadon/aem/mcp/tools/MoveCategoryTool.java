...
public class MoveCategoryTool implements McpTool {
    ...
    public String description() {
        return "Move a category folder (and all products under it) to a new catalog-relative path. "
                + "The target's parent folder must already exist.";
    }
    ...
    public String call(ResourceResolver resolver, JsonObject args) throws Exception {
        ...
        String from = ToolArgs.require(args, "from");
        String to = ToolArgs.require(args, "to");
        authoring.moveCategory(resolver, catalog, from, to);
        return "moved category '" + from + "' to '" + to + "'";
    }
}
